...
import { AlertTriangle, Check, Cloud, Download, Keyboard, UnfoldHorizontal, RotateCcw } from "lucide-react";
...
export function Toolbar(props: Props) {
  ...
  // Track last device per platform so tabs preserve user's choice.
  const lastByPlatform = React.useRef<{ ios: Device; android: Device; games: Device }>({
    ios: platform === "ios" ? props.device : "iphone",
    android: platform === "android" ? props.device : "android",
    games: platform === "games" ? props.device : "game-iphone",
  });
  React.useEffect(() => {
    if (platform === "ios" || platform === "android" || platform === "games") {
    ...
    <div className="flex flex-wrap items-center gap-x-2 gap-y-1.5 border-b bg-card/40 px-4 py-2">
      ...
      >
        <TabsList className="h-8 p-0.5">
          ...
          <TabsTrigger value="games" className="h-7 px-3 text-xs" disabled={props.busy}>
            Games
          </TabsTrigger>
        </TabsList>
      </Tabs>
      ...
      >
        ...
        <SelectContent>
          {platform === "ios" ? (
            ...
            <>
              <SelectItem value="game-iphone">{DEVICE_LABEL["game-iphone"]}</SelectItem>
              <SelectItem value="game-android">{DEVICE_LABEL["game-android"]}</SelectItem>
            </>
          ) : (
            <>
      ...
      {hasLandscape && !isGameDevice(props.device) && (
        <Select
      ...
      <div className="ml-auto flex shrink-0 items-center gap-2">
        ...
        <Button
          ...
          disabled={props.busy || !props.onShowShortcuts}
        >

...
import { isBuiltInElementId, isDecorElementId, isTextElementId, decorElementKey, textElementKey } from "@/lib/elements";
import {
  deleteElementPatch,
  ...
  isTypingTarget,
  listPresentElementIds,
} from "@/lib/element-actions";
import { resolveExportBackgroundColor } from "@/lib/background";
...
export function ScreenshotEditor() {
  ...
  const selectElement = React.useCallback((slideId: string, elementId: ElementId | null) => {
    if (!elementId) {
    ...
    setSelectedElement((prev) => {
      ...
      const isDevice = slide ? isDeviceLikeElement(slide, elementId) : false;
      const sameDevice =
        isDevice &&
      ...
      return {
        ...
        elementId,
        ...(isDevice
          ? { deviceEditMode: sameDevice ? prev?.deviceEditMode ?? "frame" : "frame" }
          : {}),
      };
    });
  }, [currentSlides]);

  const handleSelectElement = React.useCallback((element: SelectedElement | null) => {
    if (!element) {
      setSelectedElement(null);
      return;
    }
    selectElement(element.slideId, element.elementId);
  }, [selectElement]);

  const deviceEditMode =
    selectedElement?.slideId === activeSlide?.id &&
    activeSlide &&
    isDeviceLikeElement(activeSlide, selectedElement.elementId)
  ...
  const assetPaths = React.useMemo(() => {
    ...
    for (const s of allSlides) {
      ...
      for (const decor of s.decorElements || []) {
        if (decor.src && !decor.src.startsWith("data:")) paths.add(decor.src);
      }
    }
    for (const frame of state.customFrames || []) {
      if (frame.src && !frame.src.startsWith("data:")) paths.add(frame.src);
    }
    return Array.from(paths).sort();
  }, [state.slidesByDevice, state.appIcon, state.locales, state.customFrames]);
      ...
      setState((prev) => ({
        ...
        slidesByDevice: {
          ...
          [prev.device]: (prev.slidesByDevice[prev.device] || []).map((slide) => {
            ...
            if (isDecorElementId(elementId)) {
              const decorId = decorElementKey(elementId);
              return {
                ...slide,
                decorElements: (slide.decorElements || []).map((element) =>
                  element.id === decorId ? { ...element, transform } : element,
                ),
              };
            }
    ...
    (id: string) => {
      ...
      setState((prev) => {
        ...
        const copy: Slide = {
          ...
          decorElements: src.decorElements?.map((element) => ({
            ...element,
            id: nid(),
            transform: { ...element.transform },
          })),
          background: src.background ? { ...src.background } : undefined,
        };
  ...
  const deleteSelectedElement = React.useCallback(() => {
    if (!activeSlide || !selectedElement || selectedElement.slideId !== activeSlide.id) return false;
    if (!isDeletableElement(selectedElement.elementId, activeSlide)) return false;
    const patch = deleteElementPatch(activeSlide, selectedElement.elementId);
    if (!patch) return false;
    patchSlide(activeSlide.id, patch);
    setSelectedElement(null);
    return true;
  }, [activeSlide, selectedElement, patchSlide]);

  const duplicateSelectedElement = React.useCallback(() => {
    if (!activeSlide || !selectedElement || selectedElement.slideId !== activeSlide.id) return false;
    if (!isDuplicatableElement(selectedElement.elementId, activeSlide)) return false;
    const { cW } = getCanvas(state.device, state.orientation);
    ...
    const result = duplicateElementPatch(
      activeSlide,
      selectedElement.elementId,
      nid(),
      Math.max(24, cW * 0.02),
      transform ? { transform, locale: state.locale } : undefined,
    );
    if (!result) return false;
    patchSlide(activeSlide.id, result.patch);
    setSelectedElement({
      slideId: activeSlide.id,
      elementId: result.selectId,
      ...(activeSlide && isDeviceLikeElement(activeSlide, result.selectId)
        ? { deviceEditMode: selectedElement.deviceEditMode }
        : {}),
    });
    return true;
  }, [activeSlide, selectedElement, patchSlide, state.device, state.orientation, state.locale]);

  const reorderSelectedElement = React.useCallback(
    (dir: "front" | "back" | "up" | "down") => {
      if (!activeSlide || !selectedElement || selectedElement.slideId !== activeSlide.id) return;
      const id = selectedElement.elementId;
      const present = listPresentElementIds(activeSlide);
      const transforms = activeSlide.transforms || {};
      const ranked = [...present].sort((a, b) => {
        const za =
          getElementTransform(activeSlide, state.device, state.orientation, a)?.zIndex ??
          (isDecorElementId(a) ? 2 : isTextElementId(a) ? 5 : a === "deviceSecondary" ? 2 : a === "device" ? 3 : 4);
        const zb =
          getElementTransform(activeSlide, state.device, state.orientation, b)?.zIndex ??
          (isDecorElementId(b) ? 2 : isTextElementId(b) ? 5 : b === "deviceSecondary" ? 2 : b === "device" ? 3 : 4);
        return za - zb;
      });
      ...
      const nextTextElements = (activeSlide.textElements || []).map((element) => ({
        ...element,
        transform: { ...element.transform },
      }));
      const nextDecorElements = (activeSlide.decorElements || []).map((element) => ({
        ...element,
        transform: { ...element.transform },
      }));
      ranked.forEach((eid, i) => {
        const cur = getElementTransform(activeSlide, state.device, state.orientation, eid);
        if (!cur) return;
    ...
    [activeSlide, selectedElement, patchSlide, state.device, state.orientation],
  );

  const setDeviceEditMode = React.useCallback((mode: DeviceEditMode) => {
    if (!selectedElement || !activeSlide || selectedElement.slideId !== activeSlide.id) return;
    if (!isDeviceLikeElement(activeSlide, selectedElement.elementId)) return;
  ...
  React.useEffect(() => {
    function onKey(e: KeyboardEvent) {
      ...
      if (
        ...
        selectedOnActive &&
        isDeletableElement(selectedOnActive.elementId, activeSlide ?? undefined)
      ) {
        if (!mod) {
          e.preventDefault();
      ...
      if (selectedOnActive && (e.key === "f" || e.key === "F") && !mod && !e.altKey) {
        e.preventDefault();
        setDeviceEditMode("frame");
        return;
      }
      if (selectedOnActive && (e.key === "s" || e.key === "S") && !mod && !e.altKey) {
        e.preventDefault();
        setDeviceEditMode("screenshot");
        return;
      }

      if (selectedOnActive && e.key === "]") {
        e.preventDefault();
        reorderSelectedElement(mod ? "front" : "up");
        return;
      }
      if (selectedOnActive && e.key === "[") {
        e.preventDefault();
        reorderSelectedElement(mod ? "back" : "down");
        return;
      }
      ...
      if (e.key === "ArrowDown" || (e.key === "j" && !mod)) {
        ...
        e.preventDefault();
        if (selectedOnActive && duplicateSelectedElement()) {
          return;
        }
    ...
    for (const locale of locales) {
      ...
      for (const size of sizes) {
        for (let i = 0; i < currentSlides.length; i++) {
          ...
          try {
            const bg = resolveExportBackgroundColor(slide.background, theme, slide.inverted);
            const dataUrl = await captureSlide(el, cW, cH, size.w, size.h, bg);
